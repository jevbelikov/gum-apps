...
prog decaf = showMove (head game)
	       ...
	       game = if decaf == 0 
			...
			else alternate testDim 2 decaf X maxE minE testBoard

-- X to play: find the best move
solve :: Int -> Int -> Board -> Int -> String
solve depth thresh board dim
  = unlines
  . map showMove
  . take 1
  . alternate dim depth thresh X maxE minE $ board
...
randomBoard boardDim moves =  do
  ...
  return $ play moves (zip xs ys) (cycle [X,O]) (initialBoard boardDim)
